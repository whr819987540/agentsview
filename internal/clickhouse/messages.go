...
func (s *Store) GetInputOutline(
	...
	rows, err := s.queryContext(ctx, `
		...
		WHERE session_id = ?
			AND role = 'user'
			AND is_system = false
			AND `+db.ClickHouseSystemPrefixSQL("content", "role")+`
		ORDER BY ordinal`, sessionID)
	if err != nil {
		return nil, fmt.Errorf("querying clickhouse input outline: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("scanning clickhouse input outline: %w", err)
		}
		item.Timestamp = formatDBTime(ts)
		items = append(items, item)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterating clickhouse input outline: %w", err)
	}
	return items, nil
}
