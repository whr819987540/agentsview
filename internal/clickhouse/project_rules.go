...
// and disabled, across every source archive mirrored into this ClickHouse
// store, each annotated with its current governed-session count. It
// mirrors internal/db.ListProjectRules and the DuckDB version with the
// same multi-archive shape: unlike SQLite (one archive, so one rule per
// (machine, path_prefix)), two archives can each have their own rule for
// the same (machine, path_prefix) pair, and both appear here with their
// own SourceArchiveID and their own governed count, keyed by
...
func (s *Store) ListProjectRules(ctx context.Context, machine string) (db.ProjectRules, error) {
	...
	if err != nil {
		return db.ProjectRules{}, fmt.Errorf("listing clickhouse project rule machines: %w", err)
	}
...
func (s *Store) projectRulesMachines(ctx context.Context) ([]string, error) {
	rows, err := s.queryContext(ctx, `
		...
		ORDER BY machine`)
	if err != nil {
		return nil, fmt.Errorf("listing clickhouse project rule machines: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&machine); err != nil {
			return nil, fmt.Errorf("scanning clickhouse project rule machine: %w", err)
		}
		machines = append(machines, machine)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterating clickhouse project rule machines: %w", err)
	}
	return machines, nil
}
